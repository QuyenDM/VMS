...
public class DateTimeUtils {
    ...
    public static final String dateTime = "(0?[1-9]|[12][0-9]|3[01])/(0?[1-9]|1[012])/((19|20)\\d\\d) ([01]\\d|2[0-3]):[0-5]\\d:[0-5]\\d";
    ...
    public static Date dateTimeCompare(String inputDate) throws Exception {

        if (DataUtil.isStringNullOrEmpty(inputDate)) {
            return null;
        }
        Date dateValue = null;
        String tempDate = inputDate.trim();
        if (tempDate.matches(date)) {
            dateValue = convertStringToDateStandard(tempDate);
        } else if (tempDate.matches(dateTime)) {
            dateValue = convertStringToDateTime(tempDate);
        } else {
            return null;
        }
        return dateValue;
